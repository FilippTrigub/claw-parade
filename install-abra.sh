...
AGENT_WORKSPACE="${OPENCLAW_STATE_DIR}/workspace-${AGENT_NAME}"
SKILLS_DEST="${AGENT_WORKSPACE}/skills"
...
if [ ! -d "${OPENCLAW_STATE_DIR}/agents/${AGENT_NAME}" ]; then
    openclaw agents add "${AGENT_NAME}" 2>/dev/null || true
fi

mkdir -p "${AGENT_WORKSPACE}"
cat > "${AGENT_WORKSPACE}/SOUL.md" << 'SOUL'
...
if ! jq -e ".agents.list[] | select(.id == \"${AGENT_NAME}\")" "${CONFIG_FILE}" >/dev/null 2>&1; then
    AGENT_JSON=$(jq -n --arg id "${AGENT_NAME}" --arg name "${AGENT_DISPLAY_NAME}" --arg ws "${AGENT_WORKSPACE}" '{id: $id, name: $name, workspace: $ws}')
