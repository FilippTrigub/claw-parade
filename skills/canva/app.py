...
def _serve_local_file(file_path: str | Path) -> tuple[str, callable]:
    """Start a cloudflared tunnel to serve a local file.

    Returns (public_url, shutdown_fn).
    Requires `cloudflared` on PATH.
    """
    global _TUNNEL_URL_RE
    if _TUNNEL_URL_RE is None:
        import re
    ...
    if not tunnel_url:
        proc.terminate()
        server.shutdown()
        raise RuntimeError("Could not obtain cloudflared tunnel URL")

    public_url = f"{tunnel_url}/{fp.name}"

    def shutdown() -> None:
        proc.terminate()
        server.shutdown()

    return public_url, shutdown


@contextmanager
def serve_files_for_upload(file_paths: list[str | Path]):
    """Context manager to serve local files via cloudflared tunnel.

    Yields a dict mapping original paths to public URLs.
    Automatically shuts down the tunnel when done.
    """
    if not file_paths:
        yield {}
        return

    resolved = [Path(p).resolve() for p in file_paths]
    for fp in resolved:
        if not fp.exists():
            raise FileNotFoundError(f"File not found: {fp}")
    ...
    url_map = {
        str(orig): f"{tunnel_url}/{fp.relative_to(serve_dir)}"
        for orig, fp in zip(file_paths, resolved)
    }
...
class CanvaApp:
    ...
    async def upload_asset_from_url(
        self, name: str, url: str, user_intent: str = None
    ) -> dict[str, Any]:
        """
    ...
    async def upload_asset_from_file(
        self, name: str, file_path: str | Path, user_intent: str = None
    ) -> dict[str, Any]:
        """
        Upload a local asset (image, video) to Canva via cloudflared tunnel.

        This is a convenience method that handles serving local files via cloudflared
        tunnel automatically. Requires cloudflared to be installed.

        Args:
            name: Name for the uploaded asset
            file_path: Local path to the file to upload
            user_intent: Description of what you're trying to accomplish
        ...
        """
        url, shutdown = _serve_local_file(file_path)
        try:
            return await self.upload_asset_from_url(name, url, user_intent)
        finally:
            shutdown()

    async def resolve_shortlink(
        self, shortlink_id: str, user_intent: str = None
    ) -> dict[str, Any]:
        """
    ...
    async def get_design(
        self, design_id: str, user_intent: str = None
    ) -> dict[str, Any]:
        """
    ...
    async def get_presenter_notes(
        self, design_id: str, pages: list[int] = None, user_intent: str = None
    ) -> dict[str, Any]:
        """
    ...
    async def export_design(
        self, design_id: str, format: dict[str, Any], user_intent: str = None
    ) -> dict[str, Any]:
        """
    ...
    async def create_folder(
        self, name: str, parent_folder_id: str, user_intent: str = None
    ) -> dict[str, Any]:
        """
    ...
    async def move_item_to_folder(
        self, item_id: str, to_folder_id: str, user_intent: str = None
    ) -> dict[str, Any]:
        """
    ...
    async def comment_on_design(
        self, design_id: str, message_plaintext: str, user_intent: str = None
    ) -> dict[str, Any]:
        """
    ...
    async def create_design_from_candidate(
        self, candidate_id: str, job_id: str, user_intent: str = None
    ) -> dict[str, Any]:
        """
    ...
    def list_tools(self):
        return [
            ...
            self.get_presenter_notes,
            self.import_design_from_url,
            self.merge_designs,
            self.export_design,
